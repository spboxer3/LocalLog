...
// Settings Cache
let settings = { blacklist: [], limits: {}, categories: {}, categoryLimits: {}, forWorkCategories: [] };
let focusMode = false;
let notifiedDomains = {};
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'force_save') {
    ...
    saveData().then(() => { sendResponse({ status: 'saved' }); });
    return true; // Async response
  } else if (request.action === 'get_live_data') {
    // Return current memory buffer immediately (Sync response)
    sendResponse({ unsavedData: unsavedData });
    return false;
  }
});

if (!trackingInterval) { trackingInterval = setInterval(tick, 1000); }
...
function tick() {
  ...
  if (!unsavedData[currentUrl]) {
    unsavedData[currentUrl] = { seconds: 0, title: currentTitle, lastVisit: Date.now() };
  }
  ...
  // --- 3. Periodic Save ---
  const now = Date.now();
  if (now - lastSaveTime >= SAVE_INTERVAL) { saveData(); }
}
...
function checkFocusViolation(hostname) {
  ...
  const isForWork = settings.forWorkCategories && settings.forWorkCategories.includes(category);
  ...
  if (!isForWork) {
    chrome.notifications.create({
      ...
      message: `${chrome.i18n.getMessage('focusWarningMsg') || "Focus Mode ON. Avoid:"} ${hostname}`,
      priority: 2
    });
...
async function saveData() {
  ...
  try {
    ...
    for (const [url, info] of Object.entries(unsavedData)) {
      if (!dailyData[url]) dailyData[url] = { seconds: 0, lastVisit: 0, title: info.title };
...
async function forceSave() { await saveData(); }
...
async function checkLimits(hostname, dailyData) {
  ...
  if (usedMinutes >= limitMinutes && !notifiedDomains[hostname]) {
    chrome.notifications.create({
      type: 'basic',
      iconUrl: 'icons/icon128.png',
      title: chrome.i18n.getMessage('limitExceededTitle') || "Time Limit Exceeded",
      message: `${chrome.i18n.getMessage('limitExceededMsg') || "Limit reached for:"} ${hostname}`,
      priority: 2
    });
...
async function loadSettings() {
  ...
  else { settings = { blacklist: [], limits: {}, categories: {} }; }
...
function getHostname(url) { try { return new URL(url).hostname; } catch (e) { return url; } }
...
function setupBadge() { chrome.action.setBadgeBackgroundColor({ color: '#0A84FF' }); }
...
chrome.windows.onFocusChanged.addListener(async (windowId) => {
  await forceSave();
  if (windowId === chrome.windows.WINDOW_ID_NONE) { isWindowFocused = false; } 
  else {
