document.addEventListener("DOMContentLoaded", async () => {
  ...
  // --- NEW: Open Console Button ---
  const btnConsole = document.getElementById("btnOpenConsole");
  if (btnConsole) {
  ...
  document.addEventListener("visibilitychange", async () => {
    if (document.visibilityState === "visible") {
      ...
      const currentRange = activeTimeBtn
        ? activeTimeBtn.getAttribute("data-range")
        : "week";
  ...
  if (btnClearData) {
    btnClearData.addEventListener("click", async () => {
      const confirmed = await openConfirm(
        ...
        chrome.i18n.getMessage("confirmClearDataMsg") ||
          "This will permanently delete all your tracking history. Settings will be kept."
      );
      ...
      if (confirmed) {
        ...
        if (keysToRemove.length > 0) {
          ...
          showToast(
            chrome.i18n.getMessage("dataCleared") || "All history cleared.",
            "success"
          );
          ...
          showToast(
            chrome.i18n.getMessage("noData") || "No data to clear",
            "info"
          );
        }
      }
...
function resetModalListeners(btnSave, btnCancel) {
  if (!btnSave || !btnCancel)
    return { newBtnSave: btnSave, newBtnCancel: btnCancel };
...
function openModal(title, description, initialValue) {
  return new Promise((resolve) => {
    ...
    const { newBtnSave, newBtnCancel } = resetModalListeners(
      btnSave,
      btnCancel
    );
  ...
  return new Promise((resolve) => {
    ...
    if (newBtnSave) {
      newBtnSave.addEventListener("click", () => {
        ...
        resolve({
          domain: newDomain,
          limit: isNaN(newLimitVal) ? 0 : newLimitVal,
        });
      });
    }
...
function renderRules(settings) {
  ...
  sortedCategories.forEach((cat) => {
    ...
    const groupLimit =
      settings.categoryLimits && settings.categoryLimits[cat]
        ? settings.categoryLimits[cat]
        : 0;
    const groupLimitText = groupLimit > 0 ? `${groupLimit}m` : "No Limit";

    const isForWork =
      settings.forWorkCategories && settings.forWorkCategories.includes(cat);
      ...
      <div class="group-actions">
        <div class="work-toggle-container" title="Mark as Work (No Focus Warnings)">
           <label class="switch-small">
             <input type="checkbox" class="work-switch" ${
               isForWork ? "checked" : ""
             }>
    ...
    if (workSwitch) {
      workSwitch.addEventListener("change", async (e) => {
        ...
        if (checked) {
          ...
          settings.forWorkCategories = settings.forWorkCategories.filter(
            (c) => c !== cat
          );
        }
        await saveSettings(settings);
      ...
      itemEl.innerHTML = `
        <div class="rule-info">
          ...
          <div class="rule-details">
            ${
              limit > 0
                ? `<span class="tag"><i class="fa-solid fa-clock"></i> ${limit}m (Priority)</span>`
                : '<span class="tag" style="opacity:0.5">Inherit</span>'
            }
          </div>
        </div>
    ...
    try {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!updates[dateKey][url]) {
          ...
          updates[dateKey][url].seconds = Math.max(
            updates[dateKey][url].seconds,
            seconds
          );
        }
        ...
        if (domain && category && category !== "Uncategorized") {
          if (
            !settings.categories[domain] ||
            settings.categories[domain] === "Uncategorized"
          ) {
      ...
      for (const [dateKey, newUrlData] of Object.entries(updates)) {
        ...
        for (const [url, info] of Object.entries(newUrlData)) {
          if (!dailyData[url]) {
            ...
            dailyData[url].seconds = Math.max(
              dailyData[url].seconds,
              info.seconds
            );
          }
        }
